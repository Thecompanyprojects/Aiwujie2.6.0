...
-(void)getinfoData{
    ...
    [manager POST:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (integer == 2000) {
            ...
            [self.infoDic setDictionary:responseObject[@"data"]];

            //用于进行删除添加图片操作的数组
            [_selectedPhotos addObjectsFromArray:responseObject[@"data"][@"match_photo"]];
            
            //用于对比是否图片更改的原始数组
            [_pictureArray addObjectsFromArray:responseObject[@"data"][@"pic"]];
            
            _oldContent = responseObject[@"data"][@"match_introduce"];
...
-(void)uploadpic:(NSString *)path{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"is_admin"] intValue] == 1) {
        ...
        url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Power/editStep2Info"];
        
    }else{
        
        url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Matchmaker/editStep2Info"];
    }
