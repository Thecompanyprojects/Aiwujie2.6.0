...
- (void)viewDidLoad {
    ...
    switch (_unlockType) {
        ...
        case WUUnlockTypeValidatePwd:
        {
            ...
            [manager POST:[NSString stringWithFormat:@"%@%@",URL,@"Api/Other/getHeadAndNickname"] parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                
                NSInteger integer = [[responseObject objectForKey:@"retcode"] integerValue];
                //                NSLog(@"%@",responseObject);
                if (integer == 2000) {
                    
                    NSArray *array = [responseObject[@"data"][@"nickname"] componentsSeparatedByString:@"["];
                    
                    self.nameLabel.text = [NSString stringWithFormat:@"Hi,%@",array[0]];
                    
                    [self.headIconImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",responseObject[@"data"][@"head_pic"]]]];
