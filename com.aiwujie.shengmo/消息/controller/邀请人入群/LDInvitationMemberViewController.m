...
-(void)completeButtonOnClick{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Restrict/inviteOneIntoGroup"];
...
//        NSLog(@"%@",responseObject);
        ...
        if (integer != 2000) {
            ...
            NSString *warnString = [NSString string];
            
            if (_allStorageArray.count != 0 && contentVC0.storageGroupArray.count != 0) {
                
                warnString = [NSString stringWithFormat:@"已成功发送给用户%ld人，群组%ld个",_allStorageArray.count,contentVC0.storageGroupArray.count];
                
            }else if (_allStorageArray.count != 0 && contentVC0.storageGroupArray.count == 0){
                
                warnString = [NSString stringWithFormat:@"已成功发送给用户%ld人",_allStorageArray.count];

            }else if (_allStorageArray.count == 0 && contentVC0.storageGroupArray.count != 0){
                
                warnString = [NSString stringWithFormat:@"已成功发送给群组%ld个",contentVC0.storageGroupArray.count];
            }
            
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:warnString    preferredStyle:UIAlertControllerStyleAlert];
