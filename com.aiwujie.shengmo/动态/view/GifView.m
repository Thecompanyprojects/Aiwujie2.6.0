...
-(void)createOwnSystemGifData:(CGRect)frame{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Users/getMyBasicGift"];
...
-(void)createData{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Users/getmywallet"];
    ...
    [manager POST:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (integer != 2000) {
            ...
            [AlertTool alertWithViewController:_viewController andTitle:@"提示" andMessage:[responseObject objectForKey:@"msg"]];
            
            
        }else{
            
            self.beansLabel.text = [NSString stringWithFormat:@"我的魔豆: %@",responseObject[@"data"][@"wallet"]];
...
-(void)createUI:(CGRect)frame{
    ...
    [_giveSystemGifButton setTitle:@"送系统礼物" forState:UIControlStateNormal];
       ...
       MBProgressHUD *HUD = [MBProgressHUD showHUDAddedTo:_viewController.view animated:YES];
        ...
        if ([_sign isEqualToString:@"赠送给某人"]) {
            ...
            if (_isSelect) {
                 ...
                 url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",URL,@"Api/friend/RewardOne"]];
            }else{
                
                url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",URL,@"Api/friend/RewardOneBasicG"]];
            }
            
            d = @{@"uid":[[NSUserDefaults standardUserDefaults] objectForKey:@"uid"], @"fuid":self.userId,@"beans":[NSString stringWithFormat:@"%d",[_money intValue] * [_buyField.text intValue]],@"type":_psid,@"num":_buyField.text};
            
        }else{
            
            if (_isSelect) {
                
                url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",URL,@"Api/Dynamic/RewardDynamicnew"]];
                
            }else{
            
                url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",URL,@"Api/Dynamic/RewardBasicGDynamic"]];
            }

            d = @{@"uid":[[NSUserDefaults standardUserDefaults] objectForKey:@"uid"], @"did":_did,@"amount":[NSString stringWithFormat:@"%d",[_money intValue] * [_buyField.text intValue]],@"psid":_psid,@"num":_buyField.text};
        }
...
-(void)buttonClick1:(UIButton *)button{
    ...
    warnLabel.text = @"总价值达到500魔豆及以上可发大喇叭";
    ...
    cancelButton.backgroundColor = CDCOLOR;
    ...
    sureButton.backgroundColor = CDCOLOR;
