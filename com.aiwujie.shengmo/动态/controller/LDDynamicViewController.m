...
@property (nonatomic, strong) LDDynamicPageViewController *dynamicPageViewController;
...
//大喇叭右上角的红点
@property (nonatomic,strong) UIView *hornDogView;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"hornDog"] length] == 0) {
...
-(void)createHornUnreadData{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",URL,@"Api/Dynamic/getBigPresentNum"];
    ...
    [manager POST:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (integer == 3001){
            ...
            [[NSUserDefaults standardUserDefaults] setObject:@"yes" forKey:@"hornDog"];
            
        }else if (integer == 3002){
        
            _hornDogView.hidden = YES;
...
-(void)createSearchView{
    ...
    titleArray = @[@[@"筛选仅对“广场”生效(vip)"],@[@"所有人",@"男",@"女",@"CDTS"],@"根据我的性取向展示感兴趣的动态"];
...
-(void)createPageViewController{
    ...
    LDDynamicPageViewController *initialViewController = [self viewControllerAtIndex:0];// 得到第一页
...
-(void)createTopTopic{
    ...
    _topicView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, 40)];
    
    NSArray *array = @[@"推荐",@"观点",@"兴趣",@"爆照",@"交友",@"生活",@"情感",@"官方"];
...
-(void)btnButtonClick:(UIButton *)button{
    ...
    LDDynamicPageViewController *initialViewController = [self viewControllerAtIndex:button.tag + 3 - 300];// 得到对应页
...
- (void)pageViewController:(UIPageViewController *)pageViewController willTransitionToViewControllers:(NSArray<UIViewController *> *)pendingViewControllers{
    ...
    _dynamicPageViewController = (LDDynamicPageViewController *)pendingViewControllers[0];
...
- (LDDynamicPageViewController *)viewControllerAtIndex:(NSUInteger)index {
...
-(void)createNavButtn{
    ...
    _hotButton.tag = 100;
    [_hotButton setTitle:@"推荐" forState:UIControlStateNormal];
    ...
    _mineButton.tag = 102;
    [_mineButton setTitle:@"好友" forState:UIControlStateNormal];
...
-(void)navButtonClick:(UIButton *)button{
    ...
    if (button.tag == 103) {
        ...
        LDDynamicPageViewController *initialViewController = [self viewControllerAtIndex:_topicIndex];// 得到对应页
        ...
        LDDynamicPageViewController *initialViewController = [self viewControllerAtIndex:button.tag - 100];// 得到对应页
...
-(void)createButton{
    ...
    if (ISIPHONEX) {
        ...
        _hornDogView = [[UIView alloc] initWithFrame:CGRectMake(39, 52, 8, 8)];
        
    }else{
        
        _hornDogView = [[UIView alloc] initWithFrame:CGRectMake(39, 28, 8, 8)];
    }
    _hornDogView.backgroundColor = [UIColor redColor];
    _hornDogView.layer.cornerRadius = 4;
    _hornDogView.hidden = YES;
    _hornDogView.clipsToBounds = YES;
    [self.navigationController.view addSubview:_hornDogView];
    
    _dynamicButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 22, 22)];
    [_dynamicButton addTarget:self action:@selector(backButtonOnClick:) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem* rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_dynamicButton];
