...
//                NSLog(@"%@",responseObject);
        _headUrl = [NSString stringWithFormat:@"%@%@",URL,responseObject[@"data"]];
...
-(void)ButtonClick{
    ...
    if ([self isEmpty:self.nameField.text] || self.nameField.text.length == 0) {
       ...
       AFHTTPSessionManager *manager = [LDAFManager sharedManager];
        ...
        NSString *url = [NSString stringWithFormat:@"%@%@",URL,@"api/users/chargeSecond"];
        ...
        [manager POST:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            if (integer != 2000) {
                ...
                if (_sexual.length == 0) {
                    ...
                    //                NSLog(@"hdhjdjhfhjddhjhjfdjh%@",_role);
